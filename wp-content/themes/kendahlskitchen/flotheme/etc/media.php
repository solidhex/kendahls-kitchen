...
function flo_gallery_metabox_add() {
	...
	foreach ($post_types as $post_type) {
		...
		if( apply_filters( 'flo_gallery_metabox_limit', true, $post_id ) ) {
			add_meta_box( 'flo_gallery_metabox', __( 'Gallery Images', 'gallery-metabox' ), 'flo_gallery_metabox', $post_type, 'normal', 'high' );
		}
	}
}
...
function flo_gallery_metabox_html($post_id) {
	...
	while( $loop->have_posts() ): 
                $loop->the_post(); 
                global $post;
		$thumbnail = wp_get_attachment_image_src( get_the_ID(), apply_filters( 'flo_gallery_metabox_image_size', 'thumbnail' ) );
		$return .= apply_filters( 'flo_gallery_metabox_output', '<img src="' . $thumbnail[0] . '" alt="' . get_the_title() . '" title="' . get_the_content() . '" /> ', $thumbnail[0] );
	endwhile; 
         wp_reset_query();
...
function flo_gallery_metabox_do_ajax_update( $form_fields, $post ) {
	$js = '<script type="text/javascript">
                ...
                jQuery.ajax({
                        ...
                        type: "POST",
                        data: "action=refresh_metabox&post_id='.$post->post_parent.'&current_post_id=" + jQuery(\'#post_ID\', top.document).val(),
	...
	$form_fields['image-size']['html'] .= $js;
	return $form_fields;
}

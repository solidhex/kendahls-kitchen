	...
	*/
	function shortcode($atts) {
		...
		if ( $hide == 'subscribe' ) {
			...
			$this->input_form_action = "<input type=\"submit\" id=\"subscribe\" name=\"subscribe\" value=\"" . esc_attr($subscribe_button_value) . "\" />";
		} else {
		...
		// build default form
		if ( $nojs == 'true' ) {
			$this->form = "<form method=\"post\" action=\"" . $url . "\"><input type=\"hidden\" name=\"ip\" value=\"" . $_SERVER['REMOTE_ADDR'] . "\" /><div><label for=\"s2email\">" . __('Your email:', 'subscribe2') . "</label><input type=\"text\" name=\"email\" id=\"s2email\" value=\"" . $value . "\" size=\"" . $size . "\" />" . $this->input_form_action . "</div></form>";
		} else {
			$this->form = "<form method=\"post\" action=\"" . $url . "\"><input type=\"hidden\" name=\"ip\" value=\"" . $_SERVER['REMOTE_ADDR'] . "\" /><div><label for=\"s2email\">" . __('Your Email Address', 'subscribe2') . "</label><input type=\"text\" name=\"email\" id=\"s2email\" value=\"" . $value . "\" size=\"" . $size . "\" />" . $this->input_form_action . "</div></form>\r\n";
		}
